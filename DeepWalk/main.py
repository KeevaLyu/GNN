...
def process(args):
    if args.format == "adjlist":
        ...
        G = graph.load_matfile(args.input, name=args.matfile_variable_name, undirected=args.undirected)
    else:
    ...
    if data_size < args.max_memory_data_size:
        ...
        model = Word2Vec(walks, vector_size=args.representation_size, window=args.window_size,
                         min_count=0,
                         sg=1, #defalut 0 - CBOW, 1 - skipgram
                         hs=1, #defalut 0 - negative sampling, 1 - hierarchica softmax
